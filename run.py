...
from src.my_model import RobertaThreePart
...
class ModelArguments:
    ...
    classify_baseline: bool = field(
        default=False,
    )
    prompt_baseline: bool = field(
        default=False,
    )
    cl_mode: bool = field(
        default=True,
    ...
    independen: bool = field(
        default=False,
    )
    noise_norm: float = field(
        default=1.5,
    )
    tem_activa: bool = field(
        default=False
    )
...
class DynamicDataTrainingArguments(DataTrainingArguments):
    ...
    context_max_length: int = field(
        default=512,
        metadata={"help": "context_max_length"}
    )
    template_max_length: int = field(
        default=15,
        metadata={"help": "template_max_length"}
    )
...
def main():
    ...
    if 'prompt' in model_args.few_shot_type:
        ...
        raise NotImplementedError
    # special_tokens = ['[false_MASK]', '[true_MASK]']
    special_tokens = []
    # Create tokenizer
    ...
    # tokenizer.add_tokens(['[false_MASK]', '[true_MASK]'], special_tokens=True)
    ...
    train_dataset_false = (
        FewShotDataset(data_args, tokenizer=tokenizer, mode="train", use_demo=("demo" in model_args.few_shot_type), false_flag=True)
    )
    set_seed(training_args.seed)

    tem_args = copy.deepcopy(model_args.__dict__)
    config.update(tem_args)
    ...
    # model.template_tower.embeddings.word_embeddings.state_dict()['weight'][-1] = a['1']
    # model.template_tower.embeddings.word_embeddings.state_dict()['weight'][-2] = a['0']
    ...
    trainer = Trainer(
        ...
        train_dataset_false=train_dataset_false
    )
    ...
    if training_args.do_eval:
        ...
        for eval_dataset in eval_datasets:
            ...
            output, res = trainer.evaluate(eval_dataset=eval_dataset)
            eval_result = output.metrics 
            label = output[1]
            # hidden_states = output.metrics["eval_soft_label_preds"]
            false_hidden_states = []
            true_hidden_states = []
            # if training_args.make_soft_label:
            #     for i in range(len(label)):
            #         if label[i] == 0:
            #             false_hidden_states.append(hidden_states[i])
            #         else:
            #             true_hidden_states.append(hidden_states[i])
            #     false_soft_label_tensor = torch.from_numpy(np.array(false_hidden_states))
            #     true_soft_label_tensor = torch.from_numpy(np.array(true_hidden_states))
            #     false_soft_label = false_soft_label_tensor.mean(0)
            #     true_soft_label = true_soft_label_tensor.mean(0)
            #     file_path = os.path.join(training_args.output_dir,"soft_label.pt")
            #     torch.save({"0":false_soft_label,"1":true_soft_label},file_path)
    ...
    if training_args.do_predict:
        ...
        for test_dataset in test_datasets:
            ...
            output, res = trainer.evaluate(eval_dataset=test_dataset)
            ...
            if trainer.is_world_master():
                with open(output_test_file, "w") as writer:
                    logger.info("***** Test rxesults {} *****".format(test_dataset.args.task_name))
